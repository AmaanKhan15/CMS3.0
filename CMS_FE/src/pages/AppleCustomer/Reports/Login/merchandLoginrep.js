...
const Dashboard = props => {
    ...
    const handleSelectAllFilters=async()=>{
        let merList = await getMerchandiserList(data.customer_id);
        merList.map((item, key) => {
            merlist.push({  label: item.FullName , value: item.id ,id:item.store_group_id})
        })
        let storGroupList = await getStoreGroupList('',data.customer_id);
        ...
        storeList.map((item) => {
         ...
         let categoryList = await getCateoryList('',data.customer_id);
        categoryList.map((item) => {
            catlist.push({ label: item.category , value: item.id })
        })
        let skuReport = await getSKUList('',data.customer_id);
        skuReport.map((item) => {
