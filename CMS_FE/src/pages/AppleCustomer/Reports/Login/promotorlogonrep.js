...
import { getSKUList,getCateoryList,getStoreList,getStoreGroupList,getPromoterList,getCityList,getDailyMerchandRepo} from "../../../../store/services/allmerchandReports";
import {getAllPromotor} from "../../../../store/services/CitySales";
import DateRangePicker from '@wojtekmaj/react-daterange-picker';
import SalesTable from "./promotorTable"
const Dashboard = props => {
    ...
    const handleSelectAllFilters=async()=>{
        let merList = await getAllPromotor();
        merList.map((item, key) => {
            merlist.push({  label: item.first_name +''+ item.last_name , value: item.id})
        })
        ...
        let storeList = await getStoreList('','',data.customer_id);
        storeList.map((item) => {
                                    ...
                                    <CardBody >
                                        <div class="card-title">
                                            <div class="d-flex justify-content-end">
                                                <Col md={6}>
                                                    <h3 class="m-0 text-gray-900">Promoter Login Report</h3>
                                                </Col>
                                                <Col md={4}></Col>
                                                <Col md={2}>
                                                </Col>
