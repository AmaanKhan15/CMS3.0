...
import { getAllAirPodsSeries } from "../../../../../store/services/AppleStore/filtersdata"
...
const BarGraph =()=>{
  ...
  const [loader,setloader]=useState(false);
   useEffect(async ()=>{    
        let  merList= await getAllAirPodsSeries(monthdata,yeardata,regiondata);  
   ...
   const onChangeLoader=async (data)=>{
    ...
    setloader(data)
    let  merList= await getAllIpadSeries(monthdata,yeardata,regiondata); 
   setseriesdata(merList)  
    if(merList){
      setloader(false)
    }
