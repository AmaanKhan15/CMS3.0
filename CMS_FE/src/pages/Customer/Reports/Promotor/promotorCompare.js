...
import { getCityList,getCategoryCitySales ,getProductCitySales,getSKUCitySales,getPromComparewithoutCust,getAllPromotor} from "../../../../store/services/CitySales"
...
const Dashboard = props => {
    ...
    const handleSelectSKU = async (selectedSKU) => {
        ...
        let skuReport = await getPromComparewithoutCust(selectedSKU.value,selectedPromo);
            console.log("Sku report data is",selectedSKU.value)
            console.log("Compare report is",skuReport)
            skuReport.map((item, key) => {
                setreportData(item)            
                item.map((item1)=>{
