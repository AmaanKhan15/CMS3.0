...
import {
  ...
  getMerchandiserList,
  getCityList,
  getDailyMerchandRepoSKU,
} from "../../../../../store/services/allmerchandReports"
import DateRangePicker from "@wojtekmaj/react-daterange-picker"
import SalesTable from "./reportTable"
import SalesBar from "./sharemarketBar"
...
const Dashboard = props => {
  ...
  const handleSelectAllFilters = async () => {
    ...
    merList.map((item, key) => {
      merlist.push({
        label: item.FullName,
        value: item.id,
        id: item.store_group_id,
      })
    })
    let storGroupList = await getStoreGroupList()
    storGroupList.map((item, key) => {
      storegrouplist.push({
        label: item.store_group_name,
        value: item.store_details,
        id: item.id,
      })
    })
  ...
  const FilterChange = async () => {
    ...
    setfilterCount(0)
    let merchandrepoList = await getDailyMerchandRepoSKU(
      selectedMerchand.value,
      selectedStore.value,
    ...
    merchandrepoList.map(item => {
      if (item.sku_detail != null) {
        console.log(
          "data is",
          item.sku_detail["shelf_qty"],
          item.sku_detail["floor_qty"],
          item.sku_detail["ref_qty"]
        )
        ...
        shlefTotal =
          parseInt(shlefTotal) + parseInt(item.sku_detail["shelf_qty"])
        floorTotal =
          parseInt(floorTotal) + parseInt(item.sku_detail["floor_qty"])
        refTotal = parseInt(refTotal) + parseInt(item.sku_detail["ref_qty"])
      }
    })
    barData.push(shlefTotal, floorTotal, refTotal)
  }
                  ...
                  <CardBody>
                    <div class="card-title">
                      <div class="d-flex justify-content-end">
                        <Col md={6}>
                          <h3 class="m-0 text-gray-900">Market Share Report</h3>
                        </Col>
                        <Col md={4}></Col>
                        <Col md={2}></Col>
                      </div>
                ...
                <Col md={12}>
                  <Card>
                    <CardBody>
                      ...
                      <SalesTable
                        report={
                          reportData?.Floordata ? reportData?.Floordata : []
                        }
                      />
                    </CardBody>
                  </Card>
