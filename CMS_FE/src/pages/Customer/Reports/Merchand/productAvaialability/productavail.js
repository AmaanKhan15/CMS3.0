...
import SalesBar from "./productavailBar"
...
const Dashboard = props => {
    ...
    const FilterChange = async () => {
        ...
        setfilterCount(0) 
        let merchandrepoList = await getDailyMerchandRepoSKU(selectedMerchand.value, selectedStore.value, selectedCategory.label, selectedCity.label, selectedSKU.value);
        setreportData(merchandrepoList)
        merchandrepoList.map((item)=>{
            if(item.sku_detail!=null ){
                console.log("data is",item.sku_detail['shelf_qty'],item.sku_detail['floor_qty'],item.sku_detail['ref_qty'])
                if(item.sku_detail['shelf_qty']==null){
                    item.sku_detail['shelf_qty']=0
                }
                if(item.sku_detail['floor_qty']==null){
                    item.sku_detail['floor_qty']=0
                }
                if(item.sku_detail['ref_qty']==null){
                    item.sku_detail['ref_qty']=0
                }
                shlefTotal=parseInt(shlefTotal)+parseInt(item.sku_detail['shelf_qty']);
                floorTotal=parseInt(floorTotal)+parseInt(item.sku_detail['floor_qty']);
                refTotal=parseInt(refTotal)+parseInt(item.sku_detail['ref_qty']);
            }
            wareTotal=parseInt(wareTotal)+parseInt(item.qty_warehouse);
            console.log("Bar dta ia",item.qty_warehouse)
        })
        console.log("Bar dta ia",shlefTotal,refTotal,floorTotal,wareTotal)
        barData.push(shlefTotal,refTotal,floorTotal,wareTotal)
                                    ...
                                    <CardBody >
                                        <div class="card-title">
                                            <div class="d-flex justify-content-end">
                                                <Col md={6}>
                                                <h3 class="m-0 text-gray-900">Product Availiablity Report</h3>
                                                </Col>
                                                <Col md={4}></Col>
                                                <Col md={2}>
                                                </Col>
