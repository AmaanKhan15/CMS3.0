...
const Cards = (props) => {
    ...
    function onComplete(merid,stname,mname) {
        history.push( {pathname : '/single-merchand/report',state:{merId:merid,storeName:stname,merchand:mname}});
    }
    return (
        <React.Fragment>  
            {data?(  
                data.map((item,key)=>(   
              item.merchand_detail!=null?
                    ...
                    <div class="card card-xl-stretch mb-xl-8 btn-rounded">
                        <div class="card-body d-flex align-items-center pt-3 pb-0">
                            ...
                            <div class="d-flex flex-column flex-grow-0-5  py-2 py-lg-13 me-2">
                                <a href="#" class="text-muted fw-normal font-size-18">{item.merchand_detail['first_name']}</a>
                                ...
                                <div className="mb-10">
                                    <button
                                        ...
                                        onClick={()=>{onComplete(item.merchand_detail['id'],item.store_detail['store_name'],item.merchand_detail['first_name'])}}
                                    >
                                        View Details
                            ...
                            <div class="d-flex justify-content-end flex-grow-0-5" >
                                <RadialChart damage={item.damage_qty}  sales={item.total_sales_qty} total={item.total_amt} warehouse={item.qty_warehouse} />
                            </div>
                        </div>
                    </div>
                </div>                
