...
import storeSample from "../../../../assets/Files/storedemo.xlsx"
...
const UploadStoreForm = (props) => {
    ...
    const handleSubmitPersosnal = async (e) => {
        e.preventDefault()
        if (store && selectedCustomer.length != 0) {
            ...
            const response = await fetch(`${URL}` + `/bulk_upload_store`, {
                method: 'POST',
                headers: {
            ...
            console.log("response data is", responseData.status)
            setLoading(true);
            setTimeout(() => {
              setLoading(false);
            }, 3000);
            if (responseData.status == 200) {
                let stores = await getStoreList(custId)

                props.Storedata(stores)

                Swal.fire({
                    icon: 'success',
                    title: 'Stores Added Succesfully',
            ...
            Swal.fire({
                ...
                text: 'Please Select File and Customer to Upload!',
                confirmButtonColor: '#306060',
                textColor: '#306060'
            })
