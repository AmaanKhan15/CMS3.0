...
import { updatePromotor } from '../../../../store/services/promotor'
...
const AddStoreForm = (props) => {
  ...
  const handleSubmitPersosnal=async(e,id)=> {
    ...
    if(document.getElementById("fName").value!= ""  &&document.getElementById("mName").value!= ""  && document.getElementById("lName").value!= ""  && document.getElementById("contactNo").value!= ""  &&  document.getElementById("address").value!= ""  && document.getElementById("area").value!= "" && document.getElementById("email").value!= "" && document.getElementById("proofno").value!= ""&& document.getElementById("city").value!= ""){
     ...
     let data=await updatePromotor(fname,mname,lname,email,address,area,poorf_no,city,selectedProof,selectedGroup,contact,username,password,id)     
     setTimeout(() => {
       setLoading(false);
     }, 2000);
        if(data.status==200){
        ...
        Swal.fire({
            icon: 'success',
            title: 'Promoter Updated Sucessfully!',
...
var myHeaders = new Headers();
  ...
  let res = await fetch(
    `${URL}`+`/promotor/`+`${location.state.id}`,
    {
      method: "get",
      headers: myHeaders,
    }
            ...
            <Row>
                <div class="w-100">
                    <div class="pb-10 pb-lg-15">
                        <h2 class="fw-bolder text-dark">Edit Promoter    
