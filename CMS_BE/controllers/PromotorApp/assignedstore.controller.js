...
exports.postRecords = async (req, res, next) => {
    try {
        ...
        const Customerdetails = await Customer.findAll({
            ...
            attributes: [[fn('concat', col('first_name'), ' ', col('last_name')), "FullName"],'is_summary']
        });
        ...
        helper.logger.debug('log message');
            ...
            if (found) {
                ...
                jwt.verify(req.headers["authorization"], config.secret, async (err, payload) => {
                    ...
                    if (payload) {
                        ...
                        //     })
                        // }
                        return res.json({ message: "Token blacklisted. User logged out.", data: login, id: payload.id, pid: payload.token_id, logout: LoggedOutUsers, black: blacklist_token });
...
exports.postFootfallChange = async (req, res, next) => {
    ...
    try {
        ...
        var startOfWeek,endOfWeek,laststartOfWeek,lastendOfWeek ;
        var lastWeekCount=req.body.week-1;
        data.forEach(async (item, key) => {
                if (item.weekno == req.body.week) {
                    startOfWeek=item.startofweek;
                    endOfWeek=item.endofweek
                }
                if(lastWeekCount==item.weekno){
                    laststartOfWeek=item.startofweek
                    lastendOfWeek=item.endofweek
                }
        })
              
        const currentWeek = await Feedback.findAll({
            attributes: [[fn('sum', col('footfall')), "current_footfall"]],
            where: getCurrentWeekCount(req,startOfWeek,endOfWeek),
        });
        const lastWeek = await Feedback.findAll({
            attributes: [[fn('sum', col('footfall')), "last_footfall"]],
            where: getLastWeekCount(req,laststartOfWeek,lastendOfWeek),
        });
        ...
        for (var i = 0; i < currentWeek.length; i++) {
            if (currentWeek[i].dataValues['current_footfall'] == null) {
                ...
                currentWeekCount = parseInt(currentWeek[i].dataValues['current_footfall']);
            }
        }
        for (var i = 0; i < lastWeek.length; i++) {
            if (currentWeek[i].dataValues['last_footfall'] == null) {
                console.log("i am herein null")
                lastWeekCount += 0;
            } else {
                lastWeekCount = parseInt(lastWeek[i].dataValues['last_footfall']);
            }
        }
        res.status(200).json({
            status: 200,
...
exports.posttotalSalesQuarter = async (req, res, next) => {
    // const t = await sequelize.transaction();
    var selling = [];
    try {
...
exports.posttotalSalesDaily = async (req, res, next) => {
    try {
        ...
        let days = 1;
        while (days <= 7) {
            var date=moment(getDateByWeek(req.body.week,req.body.year), 'YYY-MM-DD').format(`DD`);
            var check = moment(`${req.body.month}${date}${req.body.year}`, "MMDDYY").add(days - 1, 'day').toDate();
            var weekday = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
            var dayName = weekday[check.getDay()];
            var Months = moment(check, 'YYY-MM-DD').format(`DD`)
            const currWeek = await AssignedPromotorSku.findAll({
                attributes: [[fn('sum', col('total_sales_qty')), "sales"], [fn('max', col('total_sales_qty')), "max_sales"]],
                where: getTotalSalesDays(req, days),
                raw: true,
            })
            var obj = {};
            obj[Months] = currWeek;
            if (currWeek[0]['sales'] == null) {
                regWeek.push({ "day": `${Months + " " + " " + dayName}`, "sales": 0 });
            } else {
                max_sales.push(currWeek[0]['sales'])
                regWeek.push({ "day": `${Months + " " + " " + dayName}`, "sales": currWeek[0]['sales'] });
            }
            days++;
        }
        return res.status(200).json({
...
exports.posttotalSalesYear = async (req, res, next) => {
    try {
        ...
        while (month <= 12) {
            ...
            obj[Months] = currWeek;
            if (currWeek[0]['sales'] == null) {
                regWeek.push({ "month": Months, "sales": 0 });
            } else {
                max_sales.push(currWeek[0]['sales'])

                regWeek.push({ "month": Months, "sales": currWeek[0]['sales'] });
            }
            month++;
        }
        return res.status(200).json({
...
function getTotalSalesDays(req, day) {
    var whereStatement = {};
    whereStatement.createdAt = {
        [Op.gte]: moment(`${req.body.month}01`, "MMDD").add(day - 1, 'day').toDate(),
        [Op.lt]: moment(`${req.body.month}01`, "MMDD").add(day, 'day').toDate(),
    }
    return whereStatement;
}
function getRunRateFloorFilter(week) {
    var curr = new Date;
    var whereStatement = {};
    whereStatement.createdAt = {
        [Op.gte]: moment(`${curr.getMonth()}01`, "MMDD").add(week - 1, 'week').toDate(),
        [Op.lt]: moment(`${curr.getMonth()}01`, "MMDD").add(week, 'week').toDate(),
    }
    return whereStatement;
}
function getCurrentWeekCount(req,startOfWeek,endOfWeek) {
    var whereStatement = {};
...
if (req.body.customer_id)
    ...
    whereStatement.createdAt = {
        [Op.gt]: moment(startOfWeek).format('YYYY-MM-DD 00:00'),
        [Op.lte]: moment(endOfWeek).format('YYYY-MM-DD 23:59')
    }
    return whereStatement;
}
function getLastWeekCount(req,laststartOfWeek,lastendOfWeek) {
    var whereStatement = {};
    if (req.body.store_id)
    whereStatement.store_id = req.body.store_id;
if (req.body.promotor_id)
...
if (req.body.customer_id)
    ...
    whereStatement.createdAt = {
        [Op.gt]: moment(laststartOfWeek).format('YYYY-MM-DD 00:00'),
        [Op.lte]: moment(lastendOfWeek).format('YYYY-MM-DD 23:59')
    }
    return whereStatement;
}
