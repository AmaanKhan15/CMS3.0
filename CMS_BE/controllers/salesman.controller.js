...
exports.getRecords = async (req, res, next) => {
    try {
        const Data = await Salesman.findAll({
            // include: [
            //   {
            //   model: Customer,
...
exports.getRecordsById = async (req, res, next) => {
    try {
        const Data = await Salesman.findAll({
            ...
            where: { id: req.params.salesmanId, is_deleted: '0' }
        });
        if (!Data) {
            return res.status(404).json({
                status: 404,
...
exports.getRecordsByCustomer = async (req, res, next) => {
    try {
        const Data = await Salesman.findAll({         
            where: { customer_id: req.params.salesmanId, is_deleted: '0'},
            include: [
                {
                model: StoreGroup,
...
exports.postRecords = async (req, res, next) => {
    ...
    gen_hash = data.digest('hex');
    console.log("password is",gen_hash)
    ...
    //         created_on: (moment().tz(TZ).utcOffset("+05:30").format()),
    //     })
    //     const Salesmandetail = await Salesman.create({
...
exports.updateRecords = async (req, res, next) => {
    ...
    gen_hash = data.digest('hex');
    try {
        const Salesmandetails = await Salesman.update({
            first_name: req.body.first_name,
            middle_name: req.body.middle_name,
            last_name: req.body.last_name,
            email: req.body.email,
            ...
            { where: { id: req.params.salesmanId } });
...
exports.updateCustomerIdforSalesmaniser=async(req,res,next)=>{
    try {
...
exports.updateStoreGroupIdforSalesmaniser=async(req,res,next)=>{
    try {
        const users=await User.update({
            store_group_id:req.body.storegroupid,
            customer_id:req.body.customer_id,
        },{where: { Salesman_id: req.params.salesmanId } });
        const Salesmandetails = await Salesman.update({
            store_group_id:req.body.store_group_id,
            updated_by: req.body.updated_by,
            updated_on: (moment().tz(TZ).utcOffset("+05:30").format()),
...
exports.deleteRecords = async (req, res, next) => {
    ...
    try {
        const details = await Salesman.update({
            is_deleted: '1'
        },
            { where: { id: salesmanId } });
        if (!details) {
...
exports.updateSuperIdRecords = async (req, res, next) => {
    try {
       ...
       const Promotordetails = await Salesman.update({
           supervisor_id: req.params.superId,
           updated_by: req.body.updated_by,
           updated_on: (moment().tz(TZ).utcOffset("+05:30").format()),
       },
