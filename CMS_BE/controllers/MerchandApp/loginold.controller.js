...
exports.postRecords = async (req, res, next) => {
  ...
  try {
    ...
    storeRecord.map((item) => {
      console.log("store api is",item.store_details)
    ...
    //   if (distance(req.body.latitude, req.body.longitude, alllocations[i].latitude, alllocations[i].longitude, "K") <= 0.1) {
        ...
        const LoginRecorddetail = await Loginrecords.create({
          ...
          merchand_id:userdata.merchand_id,
...
exports.getProfile=async(req,res,next)=>{
  try {
    const assignment = await Merchand.findAll({
        where: {            
            id: req.body.merchand_id,
        },
