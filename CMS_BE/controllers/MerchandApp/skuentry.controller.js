...
exports.getNearExpirayStock = async (req, res, next) => {
    try {
        const SKUdata = await floorProduct.findAll
            ({
                where: {
                    [Op.and]: [
    ...
    WareHousedata.map((item) => {
        ...
        Storedata.map((item1)=>{
            if (DiffDays < 30 && DiffDays>0) {
                allstore.push({ expiresIn: DiffDays+" "+`Days`,exp_date:item.exp_date,sku:item.sku_detail.sku,store_name:item1.store_name})
            }
        })
