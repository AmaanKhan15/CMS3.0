...
exports.getCustomerList = async (req, res, next) => {
    ...
    Data.map((item)=>{
        allCustomers.push({id:item.id,Name:item.company_name})
    })
    ...
    allMerchand.push({ Customer: allCustomers[i].id,Customer_Name:allCustomers[i].Name,MerchandCount: list_merchand.count,PromotorCount: list_promotor.count,StoreCount: list_store.count})
...
exports.postRecords = async (req, res, next) => {
    ...
    try {
        ...
        var Token=Utils.generateToken(req)
        res.status(200).json({
            status: 200,
